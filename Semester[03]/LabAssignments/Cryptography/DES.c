...
uint64_t des(uint64_t input, uint64_t key, char mode){
    int i, j;
    char row, column;
    uint32_t C = 0, D = 0, L = 0, R = 0, s_output = 0, f_function_res = 0, temp = 0;
    uint64_t sub_key[16] = {0}, s_input = 0;
    uint64_t permuted_choice_1 = 0, permuted_choice_2 = 0; /* 56 bits */
    uint64_t init_perm_res = 0, inv_init_perm_res = 0, pre_output = 0; /* 64 bits */
    /* initial permutation */
    for(i=0;i<64;i++){
        init_perm_res <<= 1;
    ...
    for(i=0;i<16;i++){
        ...
        if (mode == 'd') s_input = s_input ^ sub_key[15-i];
        else s_input = s_input ^ sub_key[i];
        /* S-Box Tables */
        for(j=0;j<8;j++) {
        ...
        temp = R; R = L ^ f_function_res; L = temp;
    }
...
int main(){
    int i;
    //uint64_t input = 0x9474B8E8C73BCA7D,key = 0x0000000000000000,result;
	char p[16],c[16],d[16];
	uint64_t input,key,result;
	printf ("\nEnter original message (hexadecimel format, eg: 9474B8E8C73BCA7D, must be of length 16) : "); scanf("%016llx",&input);
    printf ("\nEnter initial key (hexadecimel format, eg: 0000000000000000, must be of length 16) : "); scanf("%016llx",&key);
	printf ("\nDES Encyption: \n");
	printf ("\nOriginal message : %016llx", input);
    result = des(input,key,'e');
    printf ("\nEncrypted message : %016llx", result);
	printf ("\n\nDES Decyption: \n");
	printf ("\nCipher message : %016llx", result);
    result = des(result,key,'d');
    printf ("\nDecrypted message : %016llx", result);
    return 0;
